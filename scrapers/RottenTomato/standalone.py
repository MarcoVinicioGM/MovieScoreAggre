...
def _movie_url(movie_name: str) -> str:
    ...
    """
    # Remove 'the' from the beginning if present and clean the name
    movie_name = movie_name.lower().strip()
    if movie_name.startswith('the '):
        movie_name = movie_name[4:]
    
    # Handle special characters and spaces
    all_words = movie_name.split()
    underscored = '_'.join(word for word in all_words if word)
    
    # Try different URL variations
    urls = [
        f'https://www.rottentomatoes.com/m/{underscored}',
        f'https://www.rottentomatoes.com/m/the_{underscored}',
    ]
    
    # If there's a year in the title, also try with _year suffix
    if len(all_words) > 1 and all_words[-1].isdigit():
        year = all_words[-1]
        base_name = '_'.join(all_words[:-1])
        urls.extend([
            f'https://www.rottentomatoes.com/m/{base_name}_{year}',
            f'https://www.rottentomatoes.com/m/the_{base_name}_{year}'
        ])
    
    # Return the first URL that works
    for url in urls:
        response = requests.head(url, headers=utils.REQUEST_HEADERS)
        if response.status_code == 200:
            return url
            
    # If no URL works, return the basic version (will be handled by search fallback)
    return urls[0]
...
def movie_title(movie_name: str, content: str = None) -> str:
    ...
    # Update selector to use the new HTML structure
    title_element = soup.find('h1', {"id": "media-hero-label"})
    if title_element is None:
        raise ValueError(f"Could not find title for movie: {movie_name}")
        
    # Find the sr-text element within the h1 and get its text
    sr_text = title_element.find('sr-text')
    if sr_text is None:
        raise ValueError(f"Could not find sr-text element for movie: {movie_name}")
        
    return sr_text.text.strip()
